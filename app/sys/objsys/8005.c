...
common_attr_t common_attrs_32773;
...
size_t  ATTR_FUNC( attr_set )( const uint8_t attr_num, const void *const from )
{
    ...
    // сначала обработка общих атрибутов
    size = common_attr_set(&common_attrs_32773, attr_num, from);
    if (size != 0)
...
__report_change:
    _attr_on_change(OBJOFFSET, &common_attrs_32773, attr_num);
    return size;
}
...
size_t  ATTR_FUNC( attr_size )( const uint8_t attr_num )
{
    uint8_t i;
    // сначала обработка общих атрибутов
    size_t size = common_attr_size(&common_attrs_32773, attr_num);
    if (size != 0)
        return size;
...
void init_obj_32773()
{
    common_attrs_init(&common_attrs_32773);
}
