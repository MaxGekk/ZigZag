/*! @file   combuf.c
 *  @brief  Реализация коммуникационных буферов
 *  @author     Max Gekk
 *  @version    1
 *  @date   декабрь 2006 г.
 * */
...
static void task_combuf_recv()
{
    combuf_t cb_iter;

    while (0 <= (cb_iter = combuf_search(READY_MASK | INPUT_MASK | CREATE_MASK,
                                         READY_MASK | INPUT_MASK | CREATE_MASK
                                         ))) 
    {
        /* проверяем контрольную сумму */
        /*XXX контрольная сумма не совпадает, так как в поле control перезаписываются биты состояния*/
