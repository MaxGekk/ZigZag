...
//   OLED_puts(0, 18, 0xff, font6x9, "Node list");
   for(i = 0, empty = 1; i < 5; i++)
   {
      if(pos != -1)
      {
         ...
         {
            if((nodepos = strgGetNodeName(nodepos, nodes[i], 32, &ncfg)) < 0)
...
static void findNodes(unsigned int zone)
{
   ...
   for(zdispcfg.lastNodeInZone = zdispcfg.firstNodeInZone, zdispcfg.displayedNodesNum = 0; zdispcfg.lastNodeInZone < max_node; zdispcfg.lastNodeInZone++)
   {
      if(strgGetNode(zdispcfg.lastNodeInZone, &node) != STRG_SUCCESS)
         continue;
      if(node.zone == zone_cfg.zone)
      {
...
static inline void menu_inc_node_cnt(void)
{
   if(++zdispcfg.selectedNodeIdx >= zdispcfg.displayedNodesNum)
   {
      ...
      zdispcfg.upperNode2Disp  = menuPrevNode(zdispcfg.upperNode2Disp);
   }
}
...
static inline void menu_dec_node_cnt(void)
{
   ...
   if(!zdispcfg.selectedNodeIdx)
   {
      ...
      for(i = zdispcfg.displayedNodesNum; i--;)
         zdispcfg.upperNode2Disp  = menuPrevNode(zdispcfg.upperNode2Disp);
   }
   else
...
void disp_zones_cfg(unsigned char key)
{
   ...
   if((key == MNU_CLR_ALL) || (key == MNU_UPDATE))
   {
      ...
      if(!zdispcfg.showCurSelection)
      {
         ...
         for(i = 0; i < 5; i++)
         {
            if(strgGetZoneName(zdispcfg.upperZone2Disp + i, &namelist[i][0], 32, NULL) <= 0)
               break;
      ...
      else
      {
         ...
         if(strgGetZone(zdispcfg.curSelIdx, &zcfg) != STRG_SUCCESS)
            return;
         if(strgGetZoneName(zdispcfg.curSelIdx, &namelist[0][0], 32 * 5, &zcfg) < 0)
            return;
         if(key == MNU_UPDATE)
            OLED_clrLine(0, 9);
         ...
         {
            if((name_len = strgGetNodeName(zdispcfg.displayedNodes[i], &namelist[i][0], 32, NULL)) < 0)
               break;
