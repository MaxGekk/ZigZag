...
void Read_MBR (void)
{
   ...
   fat.partition_start = 0;
   if(mmcReadBuf (0) != MMC_SUCCESS)
      return;
   buffer = mmcGetBuffer();
   if (buffer[0] != 0 && buffer[1] != 0 && buffer[2] != 0)  //windows format
      return;
   if (buffer[510] == 0x55 && buffer[511] == 0xAA)
   {
      fat.partition_start = *((unsigned long *) &buffer[454]);
      //fat.file_system = buffer[450];
   }
   return;
}
...
//      res = mmcReadBuf(0);
      //if(mmcReadBuf(0) != MMC_SUCCESS)
      if(mmcReadBuf(fat.partition_start) != MMC_SUCCESS)
         return;
      buffer = mmcGetBuffer();
      if (buffer[510] == 0x55 && buffer[511] == 0xAA);
      {
