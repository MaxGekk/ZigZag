...
static const unsigned int fctrl_mb_slaves[] = {2, 4, 0};
static const unsigned int  fctrl_mb_sl_num = sizeof(fctrl_mb_slaves) / sizeof(fctrl_mb_slaves[0]) - 1;
...
int strgGetZoneName(unsigned int zone, char * name, unsigned int size)
{
   ...
   *name = 0;
   if(zone >= syscfg.zone_num)
      return STRG_INV_IDX;
   if(mmcReadBuf(syscfg.zone_cfg_addr + zone) != MMC_SUCCESS)
      return STRG_MMC_ERROR;
   ptr = mmcGetBuffer();
   for(i = 0, ptr += sizeof(struct ZoneCfg), --size; i < size; i++, name++, ptr++)
   {
      if(!(*ptr))
         break;
      *name = *ptr;
   ...
   *name = 0;
   ...
   if(!i)
   {
      if((i = mmcReadBlock(addr, 1, name)) != MMC_SUCCESS)
         i = -1 - i;
   }
   */
   return i;
...
{
   return fctrl_mb_sl_num;
}

int strgMBGetSlaveAddr(unsigned int pos)
{
   if(pos >= fctrl_mb_sl_num)
      return STRG_INV_IDX;
   return fctrl_mb_slaves[pos];
}
