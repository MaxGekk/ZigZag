...
usage = """usage: %prog seg_addr=<ADDRESS OF SEGMENT>"""
parser = OptionParser(usage)
parser.add_option("-a", "--address", action="store", dest="address", help="start address of segment", default = DEFAULT_ADDRESS )
parser.add_option("-s", "--size", action="store", dest="size", help="size of dump", default = 1 )
parser.add_option("-o", "--output", action="store", dest="output", help="file name for binary output", default = None )
...
if __name__ == '__main__':
    try:
        ...
        if options.output != None:
            out_file = file(options.output,"w")
        ...
        seg = parseInt(options.address)
        while seg < parseInt(options.address) + parseInt(options.size):
            sys.stderr.write("Read segment from %x ... " % seg )
            data = list( jtag.uploadData( seg, 512 ) )
            sys.stderr.flush()
            sys.stderr.write("ok\n")
